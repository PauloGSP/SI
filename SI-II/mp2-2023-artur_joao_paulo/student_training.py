...
import random
from Bomberman_Environment import Bomberman_Environment as env
from destroy_wall import dest
from clients import Client
from agentai import AgentAI
from destroy_enemies import Destroy_enemies
from RLAgent import RLAgent as RL
from state_tuple import StateTuple
from termcolor import colored
print(colored("[I] Starting the agent", "green"))
learning_rate =0.001
discount_factor =0.9
exploration_rate =1.0

# Define the mapping from actions to movement directions
action_to_move = {
    0:  "w",
    1:  "s",
    2:  "a",
...
async def agent_loop(server_address="localhost:8000", agent_name="84730"):
    #fazer loop aqui 1000
    num_iterations =   2
    for _ in range(num_iterations):
        # Collect a few steps using the policy
        if _ != 0:
            env._reset()
        async with websockets.connect(f"ws://{server_address}/player") as websocket:
            ...
            #There are 1085 avaiable postions on the board, 4 different types of lives (0,1,2,3)  a max of 9 enemies 
            state_size = 1085+4+18+800  # Define the size of the state        
            action_size = 6  # Define the number of possible actions
            previous_state=None
            init_flag=True
            while True:
                state = await Client.recv_game_state(websocket)
                if init_flag:
                    state["detonator"]=False
                    state["bomb_range"]=bomb_range
                    state["bombs"]=[]
                    for e in state["enemies"]:
                            
                            e["tag"]=enemy_to_id[e["name"]]
                    print("----------------I am the initial state----------------")
                    print(state)
                    
                    initial_state =StateTuple.get_state_tuple(state)
                    agent = RL(initial_state)
                    print(colored("[I] Created initial state. Starting Loop", "green"))
                    init_flag=False
                print(colored("[I] Received game state", "green"))
                print(state)
                ...
                #TODO acabar deste lado a conversão de action para key (segundo send) tratar da parte de atualizar o estado no treino
                state["detonator"]=False
                state["bomb_range"]=bomb_range
                for e in state["enemies"]:
                ...
                if previous_state!= state:
                    current_state = StateTuple.get_state_tuple(state)
                    print(colored("[A] Sending State","blue"))
                    print(current_state)
                    action=agent.helper(current_state)
                    print(colored("[A] Received Action","blue"))
                    print(action)
                else:
                    action = 6
                # Make a copy of the current state
                # Update the state variables based on the action
                #ALSO define here our key to send
                #Movement in bomberman is reversed first index is horizontal
                ...
                if len(pathing) == 0:
                    ...
                    pathing = []
                    key = action_to_move[action]
                    await websocket.send(
                        json.dumps({"cmd": "key", "key": key})
                    )
                    print(colored("[F] Sent Key: "+key,"green"))
                    ...
                    print(colored("[F] Sent Key: "+key,"green"))
                print(colored("[A] Waiting for next state","yellow"))
        print("Secalhar mato me") 
        agent.train()
